...
fn ufs_test() -> std::io::Result<()> {
    ...
    let dir0_dir1 = dir0.create("dir1", ATTR_DIRECTORY).unwrap();
    dir0_dir1.create("file2", ATTR_ARCHIVE);
    ...
    let filea = root_vfile.create("filea", ATTR_ARCHIVE).unwrap();
    fs_manager.read().cache_write_back();
